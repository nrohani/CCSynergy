...
def run(F_n):
    ...
    X=np.load('/scratch/08750/rzgar/Inputs/TranSynergy/TranSynergy/X.npy')
    Y=np.load('/scratch/08750/rzgar/Inputs/TranSynergy/TranSynergy/Y.npy')
    with open(setting.old_x_lengths, 'rb') as old_x_lengths:
    ...
    for train_index, test_index, test_index_2, evaluation_index, evaluation_index_2 in split_func(fold='fold', test_fold = F_n):
        
        print(np.concatenate((train_index, test_index, test_index_2, evaluation_index, evaluation_index_2)).max())
        print(X.shape)
        ...
        result=pd.DataFrame()
        result['Fold']=np.zeros([len(all_preds),])+F_n
        result['prediction']=all_preds.reshape(-1, 1)
        result['real']=all_ys.reshape(-1, 1)
        result['Index']=np.array(test_index_list[:sample_size]).reshape(-1,1)
        outFinal = pd.concat([outFinal,result])
    #outFinal.to_csv('/work2/08750/rzgar/stampede2/ChemicalChecker/TranSynergy/Results/ResultTS.csv')
    #outFinal.to_csv('/scratch/08750/rzgar/Inputs/TranSynergy/TranSynergy/Results/ResultTS.csv')
    to_export=pd.DataFrame(outFinal)
    to_export.to_csv("/scratch/08750/rzgar/Inputs/TranSynergy/TranSynergy/Results/Result_CV1_Fold"+str(F_n)+".csv")
