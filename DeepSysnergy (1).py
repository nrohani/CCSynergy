...
seedd=94
data=labels
#Change with synergy2 for second analysis
y=synergy.values
prenew=np.array([])
realnew=np.array([])
ii=0
...
def concatRefine(x_train,y_train,num):
    ...
    for x,y in zip(x_train,y_train):
        ...
        x_new[i,:]=np.concatenate((x[4387:8774],x[0:4387],x[8774:num]),axis=0)
        y_new.append(y)
        i=i+1
        x_new[i,:]=x
        y_new.append(y)
...
def concatPandas(data,out):
    ...
    for d1, d2 in zip(out[:,0],out[:,1]):
            TrainName=data[(((data.drug_a_name==d1)  &  (data.drug_b_name==d2 ))|((data.drug_b_name==d2)  & (data.drug_a_name==d1 )))]
...
for train_index, test_index in kf.split(UniqList):
    ...
    X_test, mean, std, feat_filt = normalize(X_test, mean, std, 
    ...
    for i in range(len(layers)):
        ...
        model.compile(loss='mean_squared_error', optimizer=keras.optimizers.SGD(lr=float(eta), momentum=0.5))
...
outFinal.to_csv('Prediction.csv')
