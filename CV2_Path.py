...
def runapp(i,n1,n2,n3,batch,lr,part):
    ...
    f = open("/nfs/research/petsalaki/users/rzgar/Outputs/Path_CV2_"+str(FeatureName[i])+".txt", "a")
    rmse_mean=0
    sp_mean=0
    pcc_mean=0
    ...
    for train_index, test_index in kf.split(UniqList):
        ...
        cb_check = ModelCheckpoint(('/nfs/research/petsalaki/users/rzgar/Outputs/DNN_PathCV2_'+str(part)+'_'+str(FeatureName[i])+'_Loewe'), verbose=1, monitor='val_loss',save_best_only=True, mode='auto')
        ...
        CNN_model = tf.keras.models.load_model('/nfs/research/petsalaki/users/rzgar/Outputs/DNN_PathCV2_'+str(part)+'_'+str(FeatureName[i])+'_Loewe')
...
#    np.save('/nfs/research/petsalaki/users/rzgar/CV1_Path_Pre_'+str(FeatureName[i])+'.npy', prenew)	
#
#    np.save('/nfs/research/petsalaki/users/rzgar/CV1_Path_Real_'+str(FeatureName[i])+'.npy', realnew)	
    outFinal.to_csv('/nfs/research/petsalaki/users/rzgar/Result_CV2_Path_'+str(FeatureName[i])+'.csv')
    rmse_mean=rmse_mean/5
    sp_mean=sp_mean/5
    pcc_mean=pcc_mean/5
    f.write(str(FeatureName[i])+ '  sp: '+str(sp_mean)+' rmse:'+str(rmse_mean)+' pc: '+str(pcc_mean))
    f.write('\n')
