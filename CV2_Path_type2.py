    ...
    for train_index, test_index in kf.split(UniqList):
        ...
        if ii==fold:
            if tissue!=False:
                ...
                cb_check = ModelCheckpoint(('/nfs/research/petsalaki/users/rzgar/Outputs/DNN_Path_Tissue_CV2_'+str(part)+'_'+str(FeatureName[i])+'_Loewe'), verbose=1, monitor='val_loss',save_best_only=True, mode='auto')
                ...
                CNN_model = tf.keras.models.load_model('/nfs/research/petsalaki/users/rzgar/Outputs/DNN_Path_Tissue_CV2_'+str(part)+'_'+str(FeatureName[i])+'_Loewe')
                ...
                outPut['Real']=y_test
                outPut['Pre']=pre_test
                outFinal.to_csv('/nfs/research/petsalaki/users/rzgar/ResultPath_TissueCV2_'+str(FeatureName[i])+'_'+str(ii)+'_'+str(tissue)+'.csv')
            ...
            if cellline!=False:
                ...
                #Please customize these Paths
                cb_check = ModelCheckpoint(('/nfs/research/petsalaki/users/rzgar/Outputs/DNN_PathCV2_Cell_'+str(part)+'_'+str(FeatureName[i])+'_Loewe'), verbose=1, monitor='val_loss',save_best_only=True, mode='auto')
        
                y_train=np.array(y_train)
                
                CNN_model.fit(x=X_train,y=y_train,batch_size=batch,epochs = 100,shuffle=True,validation_data = (X_val,y_val),callbacks=[EarlyStopping(monitor='val_loss', mode='auto', patience = 10),cb_check] )
        
                CNN_model = tf.keras.models.load_model('/nfs/research/petsalaki/users/rzgar/Outputs/DNN_PathCV2_Cell_'+str(part)+'_'+str(FeatureName[i])+'_Loewe')
                pre_test=CNN_model.predict(X_test)
                prenew=np.concatenate((pre_test,prenew), axis=None)
                realnew=np.concatenate((y_test,realnew), axis=None)
                outPut['Real']=y_test
                outPut['Pre']=pre_test
                outFinal.to_csv('/nfs/research/petsalaki/users/rzgar/ResultPath_CellCV2_'+str(FeatureName[i])+'_'+str(ii)+'_'+str(cellline)+'.csv')
